...
function createStore(useHooks) {
  ...
  var Context = /*#__PURE__*/react.createContext(undefined, function () {
    return 0;
  }); // @ts-ignore
  // 创建数据订阅队列，用于保存使用useSelector组件的更新函数

  var ListenerContext = /*#__PURE__*/react.createContext(new Set(), function () {
    return 0;
  });
  /**
  ...
  var Provider = function Provider(_ref) {
    ...
    var listeners = react.useContext(ListenerContext);
    ...
    return jsxRuntime.jsx(Context.Provider, {
      value: store,
      children: jsxRuntime.jsx(ListenerContext.Provider, {
        value: listeners,
        children: children
      }, void 0)
    }, void 0);
  ...
  function useSelector(selector) {
    ...
    // 强制更新
    var _useReducer = react.useReducer(function () {
      return {};
    }, {}),
    ...
    var store = react.useContext(Context); // 获取数据订阅列表

    var listeners = react.useContext(ListenerContext); // 通过ref保存上次render数据，方便 Effect 依赖为[]时可以访问最新数据，用于新老数据对比，决定是否render

    var lastSelectRef = react.useRef(); // 获取筛选数据
